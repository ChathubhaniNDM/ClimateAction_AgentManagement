...
function AllAgents() {
  ...
  // const [agents, setAgents] = useState([{ fullname: "John Doe", email: "john@example.com", phone: "123456789", district: "Sample District", description: "Sample description" }]);
  ...
  useEffect(() => {
    function getAgents() {
        axios.get("http://localhost:8070/Agent/")
            .then((res) => {
                setAgents(res.data);
                console.log("Fetched agents:", res.data);  // Add this to see if data is fetched
            })
            .catch((err) => {
                alert(err.message);
            });
...
const handleSearch = (e) => {
  ...
  console.log("Search query:", e.target.value);  // Log search query to confirm input
};
...
console.log("Filtered agents:", filteredAgents);  // Log filtered agents to debug
